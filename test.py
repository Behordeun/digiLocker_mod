...
from utils import *
import pymongo
from pymongo import MongoClient, GridFS  # Import MongoClient
...
try:
    ...
    # Create collections for users and documents
    users_collection = db["users"]
    documents_collection = db["documents"]

    # Define a user schema (you can add more fields as needed)
    user_schema = {
        "user_address": {"type": "string", "unique": True},
        "email": {"type": "string"},
        "first_name": {"type": "string"},
        "last_name": {"type": "string"},
        "utype": {"type": "string"},
        "master_key_hash": {"type": "string"},
        "public_key": {"type": "string"},
    }

    # Define a document schema (you can add more fields as needed)
    document_schema = {
    ...
    users_collection.create_index("user_address", unique=True)
    documents_collection.create_index("doc_id", unique=True)

    # Add the user and document schemas to the collections (optional)
    users_collection.insert_one(user_schema)
    documents_collection.insert_one(document_schema)

except pymongo.errors.ConnectionFailure as e:
    print(f"MongoDB connection error: {e}")
    # Handle connection error appropriately, e.g., exit the application or log an error message
...
# Replace Dropbox with MongoDB in the following line
# dropbox_ = dropbox.Dropbox(app.config["DROPBOX_ACCESS_TOKEN"])
# Use MongoDB GridFS to handle file storage
fs = GridFS(db)
...
# Modify the upload_file_postapi function to use MongoDB GridFS
    ...
    try:
        ...
        if file.filename == '':
            ...
            try:
                ...
                fs.put(fileContent, filename=docId, user_address=user_address, docHash=docHash)
            except Exception as e:
                print(e, "error")
                return {"success": False, "error": str(e)}
...
# Modify the downloadEncryptedFileNcompareHash function to use MongoDB GridFS
...
def downloadEncryptedFileNcompareHash(user_address):
    ...
    try:
        ...
        # 1. Download the file from MongoDB GridFS
        fileData = None
        try:
            doc_record = fs.find_one({"_id": ObjectId(doc_id)})
            fileData = doc_record.read()
        except Exception as e:
            print(e, "error")
            return {"success": False, "error": str(e)}

        # Compare hash
        docHashObtained = "0x" + hashlib.sha256(fileData).hexdigest()

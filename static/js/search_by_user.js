            ...
            for (var j = 0; j < documents.length; j++) {
              $("#document_table tbody").append(
                `<tr>
                        ...
                        <td><button class = "btn btn-primary sharedoc" doc_id="${
                          documents[j].doc_id
                        }"
                        doc_name="${
                          documents[j].filename
                        }">Request</button></td>
                        </tr>`,
              );
            }
...
function sendRequestMailAjaxUser(doc_id, email, doc_name) {
  ...
  contract.methods
    .getEmailIdByUsrAddr(owner_address_)
    .call()
    .then(function (result) {
      console.log(result);
          ...
          contract.methods
            ...
            .then(function (obj) {
                ...
                if (request.status >= 200 && request.status < 400) {
                  ...
                  if (resp.success) {
                    swal({
                      ...
                      if (value) window.location.replace("/dashboard");
                    });
                  }
                } else {
                  $("#main-loader").hide();
              ...
              request.onerror = function () {
                ...
                console.log(
                  "Error while raising the request(sending mail to owner) - while searching by user id/email id",
                );
              };
